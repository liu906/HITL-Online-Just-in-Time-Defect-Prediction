		...
		while (stream.hasMoreInstances() && ((maxInstances < 0) || (instancesProcessed < maxInstances))
			...
			if (write_results) {
				...
				if (instancesProcessed % this.sampleFrequencyOption.getValue() == 0
					...
					String ts = (((Instance) testInst.getData()).value(15) + ""); //get time stamp
					...
					if(idxE > 0){
						...
						((Vector<String>) hashResults.get(RES_TS))
								.add(ts); //add ts to the time stamp
						
						((Vector<Integer>) hashResults.get(RES_INSTANCE_CLASS))
								.add(new Double(((Instance) testInst.getData()).classValue()).intValue()); //get instance label and add to hashResult
						((Vector<Integer>) hashResults.get(RES_INSTANCE_PREDICTION)).add(predictedClass);
						((Vector<Double>) hashResults.get(RES_RECALL_0))
								.add(evaluator.getPerformanceMeasurements()[7].getValue()); //get recall_0 from getPerformanceMeasurements()[7] and save to hashResult
